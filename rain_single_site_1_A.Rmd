...
```{r charts-part-4, eval= FALSE,echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis',fig.width=10,fig.asp=0.618}
...
```{r gap-times-table-1, eval=TRUE, echo = FALSE,message=FALSE, warning=FALSE}
      ...
      # create a list of the saved stats files (file paths / filenames)
      stats_path_and_filenames <- list.files(stats_folder_3,full.names = TRUE)
      ...
      # read the data into a dataframe
      table_data_3 <- read.table(stats_path_and_filenames[2],sep=",",header=TRUE,stringsAsFactors = FALSE)
      
      # name the columns
      years_char <- as.character(year_as_numeric)
      names(table_data_3) <- c("Statistic",years_char)
      
      
      
        # use the 'flextable' package to create MASTER table (e.g. MIT 60); may not be very readable
       
      if (ncol(table_data_3) <= 10) {
      
        # table of all available data
        
        table_data_3 %>%  flextable() %>%
          # set_caption(caption = table_caption) %>%
          # fontsize(size = 10, part = "all") %>%
          # colformat_double(j=c(2:4),digits = 3) %>%
          colformat_double(i=c(12,13),digits = 0) %>%
      ...
      if (ncol(table_data_3) > 10 & ncol(table_data_3)<=20) {
      
        last_col <- ncol(table_data_3)
        
        # select column 10 to last 
      
        table_data_3_part_2 <- table_data_3[,c(1,10:last_col)]
      
        # use the 'flextable' package to create SECOND table (e.g. MIT 60)
      
        table_data_3_part_2 %>%  flextable() %>%
        # set_caption(caption = table_caption) %>%
        # fontsize(size = 10, part = "all") %>%
        # colformat_double(j=c(2:4),digits = 3) %>%
        colformat_double(i=c(12,13),digits = 0) %>%
       ...
       if (ncol(table_data_3) > 20 & ncol(table_data_3)<=30) {
        ...
        # select column 21 to last 
      
        table_data_3_part_3 <- table_data_3[,c(1,21:last_col)]
      
        # use the 'flextable' package to create SECOND table (e.g. MIT 60)
      
        table_data_3_part_3 %>%  flextable() %>%
        # set_caption(caption = table_caption) %>%
        # fontsize(size = 10, part = "all") %>%
        # colformat_double(j=c(2:4),digits = 3) %>%
        colformat_double(i=c(12,13),digits = 0) %>%
      ...
      if (ncol(table_data_3) > 30) {
      
        last_col <- ncol(table_data_3)
        
        # select column 31 to last 
      
        table_data_3_part_4 <- table_data_3[,c(1,31:last_col)]
      
        # use the 'flextable' package to create SECOND table (e.g. MIT 60)
      
        table_data_3_part_4 %>%  flextable() %>%
        # set_caption(caption = table_caption) %>%
        # fontsize(size = 10, part = "all") %>%
        # colformat_double(j=c(2:4),digits = 3) %>%
        colformat_double(i=c(12,13),digits = 0) %>%
...
```{r gap-times-table-2, eval=FALSE, echo = FALSE,message=FALSE, warning=FALSE}

      # SECOND TABLE OF GAP TIME RESULTS 
      
      
# FIRST TABLE OF GAP TIME RESULTS 
      
      if (ncol(table_data_3) > 12) {
      
      # select column 1 to columns 12
      
      table_data_3_part_1 <- table_data_3[,1:12]
      
       # use the 'flextable' package to create FIRST table (e.g. MIT 60)
      
        table_data_3_part_1 %>%  flextable() %>%
        # set_caption(caption = table_caption) %>%
        # fontsize(size = 10, part = "all") %>%
        # colformat_double(j=c(2:4),digits = 3) %>%
        colformat_double(i=c(12,13),digits = 0) %>%
