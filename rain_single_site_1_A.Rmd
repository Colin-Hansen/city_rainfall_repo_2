  ...
  # read a single .CSV file; generate basic summary stastics
    ...
    csv_name<-list.files('data_out/MASTER_CSV_5_min')
      ...
      bold(bold=TRUE,part = "header") %>% 
        # FIX THE FOLLOWING LINE - MISSING SOME QUOTES
   ...
   # use the 'flextable' package to create table
      ...
      set_caption(caption = "Rainfall Statistics: Hourly to Yearly [Filter: May to September]") %>% 
      font(fontname = "Calibri (Body)", part = "all") %>% 
      fontsize(size = 10, part = "body") %>% 
      # add footer if you want
      # add_footer_row(values = "* p < 0.05. ** p < 0.01. *** p < 0.001.", 
...
```{r charts-part-1, eval= FALSE,echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis',fig.width=9,fig.asp=0.618,fig.align='centre'}
